...
function fg2(ds, featurizer, r, ics; Ttr=50.0, Dt=0.5, T=100.0)
    ...
    for i in GroupCentresIndex        # trajectory
        neuron = CoupledODEs(ds, ics[i], params)
        N, t = trajectory(neuron, T; Ttr, Δt=0.2) # Discarding Ttr seconds of transient time
...
GroupIndex, GroupCentresTrajectories = fg2(ds, featurizer, r, ics; Ttr=50, Dt=0.5, T=100)
...
println(FractionsBoA) # Print the factions for each index

#println(GroupCentresTrajectories[1][1][1]) 

#=
# I need to plot full trajectories without ignoring transcient time as this is demonstrating the basins of attraction
fig = Figure()
ax = Axis(fig[:,:]; title = "Group Centre trajectories", xlabel = "v", ylabel = "w")
...
for i in 1:3 # change to a length at some point
    ...
    #println(x)
    #println(y)
lines!(ax, x, y, color = colours[i])
end
scatter!(ac, x=0, y=0, color = colour[3]) # as they are too small to be seen
fig
