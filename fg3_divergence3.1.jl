...
function fg3(ds, featurizer, r, ics; Ttr, Dt, T)#Ttr=50.0, Dt=0.5, T=100.0)
    ...
    #FractionsBoA = Vector{Float64}(undef, length(GroupCentres))
    FractionsBoA = zeros(length(GroupCentres))
    # Bains of Attraction fractions
    for i in 1:length(GroupCentres) # loop over number of attractors# find the number of elements with index i
