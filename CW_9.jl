...
function fg3(ds, featurizer, r, ics; Ttr=50.0, Dt=0.5, T=100.0)
    ...
    for i in 1:length(ics)
        # trajectory
        ODES = CoupledODEs(ds, ics[i], params)
...
# fix this section to work with an arbitrary featurizer and trajectory
    ...
    for i in GroupCentresIndex        # trajectory
        ...
        ODEs = CoupledODEs(ds, ics[i], params)
        Trajectory, t = trajectory(neuron, T; Ttr=TransientTime, Δt=0.2) # Discarding Ttr seconds of transient time
        for j in 1:length(Trajectory[1])
        #v_temp, w_temp = Trajectory[:, 1], Trajectory[:, 2] # separate by component # pretty sure these can also be removed
        #v, w = v_temp[k2:end], w_temp[k2:end] # ignore section before transient time
        push!(VecTemp, Trajectory[:, i])
        end
