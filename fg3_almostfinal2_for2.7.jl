...
function fg3(ds, featurizer, r, ics; Ttr, Dt, T)#Ttr=50.0, Dt=0.5, T=100.0)
    ...
    for InitialConditions in ics#i in 1:length(ics)
        ...
        DSystem = DeterministicIteratedMap(ds, InitialConditions, Parameters)
        Trajectory, t = trajectory(DSystem, T; Ttr, Dt, t0 = 0)#; Ttr, Δt=Dt) # Discarding Ttr seconds of transient time
        # find the features
    ...
    for i in GroupCentresIndex        # trajectory
        ...
        DynamicalSystem = DeterministicIteratedMap(ds, ics[i], Parameters)
        Trajectory, t = trajectory(DynamicalSystem, T; Ttr, Dt, t0 = 0)#; Ttr=TransientTime, Δt=0.2) # Discarding Ttr seconds of transient time
        # Use the number of columns in Trajectory for robustness
        #num_components = size(Trajectory, 2)  # double check this step again URGENT ####
        for j in 1:length(Trajectory[1, :])#num_components
